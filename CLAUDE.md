...
- [`docs/ai-features.md`](docs/ai-features.md) — how AI features work: all AI runs in a Supabase Edge Function holding the Claude key (app never calls Anthropic directly), the read-only Chat flow (computed summaries first, raw rows only when a question needs detail), and the privacy rules (least data, user-scoped, rows stay server-side, disclosed). Read when building Chat or any AI feature.
...
- [`docs/deployment.md`](docs/deployment.md) — Android release pipeline: signed AAB from Gradle (`./gradlew bundleRelease`), keystore/config as CI secrets (never committed) with a `.env.example` template, PR-to-main builds as a check only, merge-to-main uploads to the internal track via the Play Developer API (service account), and always-manual promotion to production. Read before touching CI, signing, versioning, or Play Store release steps.
